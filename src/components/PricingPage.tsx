...
import { Check, Sparkles, Shield, ArrowLeft, RefreshCw, Zap, Lock, Loader2 } from 'lucide-react';
...
export const PricingPage: React.FC<PricingPageProps> = ({ onSuccess }) => {
  ...
  const { isReady, isLoading: isSdkLoading, openCheckout } = useFreemius();
    ...
    try {
      ...
      await openCheckout({
        ...
        success: (response) => {
          ...
          navigate(`/welcome?tier=${tier.name.toLowerCase()}&cycle=month`);
        },
        cancel: () => {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100/80 text-emerald-800 text-xs font-bold mb-4">
            ...
            <span>Risk-Free 7-Day Trial on All Plans</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-neutral-900 tracking-tight">
            Predictable, transparent coaching plans
          </h1>
          <p className="mt-3 text-base text-neutral-600">
            Select the tier that fits your speaking and workplace goals. Powered securely by Freemius with 1-click self-service cancellation.
          </p>
        </div>
              ...
              >
                ...
                <div className="p-7">
                  ...
                  <button
                    ...
                    className={`mt-6 w-full py-3.5 px-4 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 bg-emerald-600 hover:bg-emerald-700 text-white shadow-md hover:shadow-lg`}
                  >
                    {isCurrentSubmitting ? (
                      <>
