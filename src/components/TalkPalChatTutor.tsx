...
export const TalkPalChatTutor: React.FC<TalkPalChatTutorProps> = ({
  ...
  return (
    <div id="talkpal-ai-chat-tutor-container" className="flex flex-col h-[calc(100dvh-130px)] min-h-[500px] max-h-[820px] sm:h-[760px] bg-white rounded-2xl sm:rounded-3xl border border-neutral-200/90 shadow-xl shadow-slate-900/5 overflow-hidden relative">
      
      {/* Top Header: Coach Selector & Speed Controls */}
      <div className="px-3 sm:px-5 py-2 sm:py-3.5 border-b border-neutral-100 bg-white/95 backdrop-blur-md flex flex-wrap items-center justify-between gap-2 sm:gap-3 shrink-0">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-indigo-50 flex items-center justify-center text-xl sm:text-2xl border border-indigo-200/80 shrink-0 shadow-2xs">
            {selectedPersona.avatar}
          </div>
          <div>
            <div className="flex items-center gap-1.5 sm:gap-2">
              <h3 className="font-extrabold text-[13px] sm:text-sm text-neutral-900 tracking-tight">{selectedPersona.name}</h3>
              <span className="px-1.5 sm:px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[9px] sm:text-[10px] font-extrabold flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                Online AI Tutor
              </span>
            </div>
            <p className="text-[10.5px] sm:text-[11px] text-neutral-500 truncate max-w-[140px] xs:max-w-[200px] sm:max-w-xs font-medium">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden md:flex items-center gap-1 bg-neutral-200/60 p-1 rounded-xl">
      ...
      <div className="px-3 sm:px-4 py-1.5 bg-indigo-50/60 border-b border-indigo-100/70 flex flex-wrap items-center justify-between gap-1.5 sm:gap-2.5 text-xs">
        
        {/* Left: Translation Language Option Dropdown */}
        <div className="flex items-center gap-1.5 sm:gap-2 relative" ref={langDropdownRef}>
          ...
          <button
            ...
            className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-xl bg-white border border-indigo-200 hover:border-indigo-400 text-indigo-950 font-bold flex items-center gap-1.5 shadow-2xs hover:bg-indigo-50/50 transition-all cursor-pointer text-xs"
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <button
            ...
            className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-xl text-xs font-extrabold flex items-center gap-1.5 transition-all cursor-pointer border ${
              autoTranslate
          ...
          >
            ...
            <span>{autoTranslate ? 'ON' : 'OFF'}</span>
          </button>
          ...
          >
            ...
            <span>Draft</span>
            <span className="hidden sm:inline"> in {currentLangObj.name}</span>
          </button>
          ...
          >
            <BookOpen className={`w-3.5 h-3.5 ${showQuickLookup ? 'text-white' : 'text-purple-600'}`} />
            <span>Translate</span>
          ...
          {mostRecentCoachMsg && (
            ...
            >
              ...
              <span>Replay</span>
              <kbd className={`hidden sm:inline px-1 py-0.2 font-mono text-[9px] rounded border font-black ${
                isRecentlyTriggered 
      ...
      <AnimatePresence>
        {showDraftHelper && (
          <motion.div
            ...
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="p-2.5 sm:p-3.5 bg-indigo-50/90 border-t border-indigo-200 space-y-2 shrink-0"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-xs font-black text-indigo-950">
                <Sparkles className="w-3.5 h-3.5 text-amber-500" />
                <span className="text-[11.5px] sm:text-xs">Draft in {currentLangObj.name} & Translate to English</span>
              </div>
              <button
                type="button"
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              <input
                ...
                className="flex-1 px-2.5 py-1.5 sm:px-3 sm:py-2 text-xs rounded-xl bg-white border border-indigo-200 focus:outline-hidden focus:border-indigo-600 font-medium text-neutral-900"
              />
              <button
                type="button"
                ...
                className="px-2.5 py-1.5 sm:px-3.5 sm:py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold flex items-center gap-1 transition-all disabled:opacity-50 cursor-pointer shadow-xs shrink-0"
              >
      ...
      {/* Message Input Bar (Framer-style unified dock) */}
      <div className="p-2 sm:p-3 bg-white/95 backdrop-blur-md border-t border-neutral-100/80 shrink-0">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleSendMessage();
          }}
          className="group relative flex items-center gap-1 sm:gap-1.5 p-1 sm:p-1.5 rounded-2xl sm:rounded-3xl bg-neutral-100/80 hover:bg-neutral-100 focus-within:bg-white border border-neutral-200/90 focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/15 transition-all shadow-[0_1px_3px_rgba(0,0,0,0.02)]"
        >
          ...
          <button
            ...
            className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all shrink-0 cursor-pointer ${
              isRecording
                ? 'bg-rose-500 text-white animate-pulse shadow-md shadow-rose-200 ring-2 ring-rose-400'
                : 'text-neutral-500 hover:text-neutral-900 hover:bg-neutral-200/60 active:scale-95'
            }`}
          ...
          <input
            ...
            placeholder={
              isRecording 
                ? 'Listening... speak now...' 
                : `Type in English or ${currentLangObj.name}...`
            }
            disabled={isLoading}
            className="flex-1 min-w-0 px-2 py-1.5 sm:py-2 bg-transparent border-0 outline-none ring-0 text-[14px] sm:text-[15px] font-medium text-neutral-900 placeholder:text-neutral-400 focus:outline-none"
          />
          ...
          <button
            ...
            className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center transition-all shrink-0 cursor-pointer ${
              input.trim()
                ? 'text-emerald-700 bg-emerald-100/90 hover:bg-emerald-200/80 active:scale-95 shadow-2xs'
                : 'text-neutral-300 opacity-50 cursor-not-allowed'
            }`}
            title={`Translate current typed text from ${currentLangObj.name} to English`}
          >
          ...
          <button
            ...
            disabled={!input.trim() || isLoading}
            className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl flex items-center justify-center font-bold transition-all shrink-0 cursor-pointer ${
              input.trim() && !isLoading
                ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-300 hover:bg-indigo-500 active:scale-90'
        ...
        {/* Quick Keyboard Helper & Audio Speed Indicator */}
        <div className="mt-1.5 hidden sm:flex items-center justify-between text-[11px] text-neutral-400 px-2">
          <div className="flex items-center gap-1.5">
            <Volume2 className="w-3 h-3 text-indigo-500 shrink-0" />
            <span>Shortcut: Press <kbd className="font-mono font-bold bg-neutral-100 text-neutral-700 px-1 py-0.5 rounded border border-neutral-200 text-[9px]">R</kbd> to replay coach audio</span>
          </div>
          <span className="font-medium text-neutral-500">Audio Speed: {speed}x</span>
        </div>
      </div>
